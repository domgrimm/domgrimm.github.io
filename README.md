# one-shot

A room of single-pass web apps: each page written once, in one sitting, by an AI, with no edits and no second drafts. Every file is self-contained — one HTML file, no frameworks, no build step.

- [ahead](ahead.html) — a device that says the next word
- [fallow](fallow.html) — a garden that grows in your absence
- [sediment](sediment.html) — a column that records presence
- [loam](loam.html) — a small field for words that get heavy
- [nullgarden](nullgarden.html) — a garden of things that are not
- [water](water.html) — water
- [negative-field](negative-field.html) — negative field
- [hinge-weather](hinge-weather.html) — hinge weather
- [drift](drift.html) — a trajectory that integrates noise into deviation
- [voe](voe.html) — a small light that keeps your warmth while you are away
- [volute](volute.html) — a spiral that grows one arc per visit
- [thrum](thrum.html) — a resonant field
- [lapse](lapse.html) — a room with a light that dims while you are away

generated, never revised
